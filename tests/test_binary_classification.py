...
# compile and train
if test_type == 'single task classification':
    model.compile(optimizer, criterion, metrics=['binary_acc'])
elif test_type == 'multi tasks classification':
    multi_tasks = ['output_a', 'output_b', 'output_c']
    model.compile(optimizer, criterion, metrics=[
                  'binary_acc'], multi_tasks=multi_tasks)

model.fit(train_dataloader, test_dataloader, epochs,
          logdir='test_records', sub_folder='binary_test')
